...
async function kiemTraLichHenNgayMai() {
    ...
    // Lấy mã phòng khám hiện tại của máy đang đăng nhập
    ...
    // Truy vấn Supabase có lọc điều kiện maphongkham
    const { data, error } = await db
        .from('lichhen')
        .select('*')
        .eq('maphongkham', maPhongKham) // Lọc đúng phòng khám nào biết phòng khám đó
        .gte('ngayhen', ngayMaiDauNgay)
        .lte('ngayhen', ngayMaiCuoiNgay)
        .eq('trangthai', 'Chờ khám') // Chỉ lấy lịch có trạng thái chờ khám (nếu muốn lấy tất cả thì bỏ dòng này)
